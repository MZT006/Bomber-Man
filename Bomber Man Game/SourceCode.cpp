    ...
    case '1':
        ...
        while (!GameOver) {
           ...
           SetConsoleCursorPosition(GetStdHandle(STD_OUTPUT_HANDLE), { 0 });
            ...
            if (NoEnemy <= 0) {
                PlaySound(TEXT("C:\\Users\\Abdul Moiz\\Downloads\\transition.wav"), NULL, SND_FILENAME | SND_ASYNC);
                level++;
                if (level > 4) {
...
void EnemyMotion(char grid[rows][cols], int EnemyX[], int EnemyY[], int& NoEnemy, int& PlayerHealth, int& BossHealth, bool& GameOver, int& score, int& Playerr2Health) {
    ...
    for (int i = 0; i < NoEnemy; i++) {
        ...
        if (direction == 0 && grid[x - 1][y] != '#' && grid[x - 1][y] != '0' && grid[x - 1][y] != 'E' && grid[x - 1][y] != 'B' && grid[x - 1][y] != 'C') {//upwards
            if (grid[x - 1][y] == 'P') {
                PlaySound(TEXT("C:\\Users\\Abdul Moiz\\Downloads\\BombRange.wav"), NULL, SND_FILENAME | SND_ASYNC);
                PlayerHealth--;
            }
            if (grid[x - 1][y] == 'Q') {
                PlaySound(TEXT("C:\\Users\\Abdul Moiz\\Downloads\\BombRange.wav"), NULL, SND_FILENAME | SND_ASYNC);
...
void PlayerMotion(char grid[rows][cols], int& x, int& y, int& key, int& PlayerHealth) {
    ...
    PlaySound(TEXT("C:\\Users\\Abdul Moiz\\Downloads\\beep.wav"), NULL, SND_FILENAME | SND_ASYNC);
    return;
}
//[END]
//========================================================================================================================================================================================================================
...
void BombExplosion(char grid[rows][cols], int& x, int& y, int& level, int EnemyX[], int EnemyY[], int& NoEnemy, int& PlayerHealth, int& BossHealth, bool& GameOver, int& Blocks, int& Bombs, int BombX, int BombY, int& score, bool& PowerUp, int& Player2Health) {
    ...
    if (Bombs > 0) {
        ...
        PlaySound(TEXT("C:\\Users\\Abdul Moiz\\Downloads\\Explosion.wav"), NULL, SND_FILENAME | SND_ASYNC);
        if (Blocks >= Power) {
            Player2Health++;
            PlayerHealth++;
            Bombs++;
...
void Menu() {
    cout << "\033[?25l";
    PlaySound(TEXT("C:\\Users\\Abdul Moiz\\Downloads\\intro.wav"), NULL, SND_FILENAME | SND_ASYNC);
    system("cls");
    level = 1;
    GameOver = false;
